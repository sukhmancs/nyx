{
  ...
  iso = modulePath + /roles/iso; # for providing a uniform ISO configuration for live systems - only the build setup
  headless = modulePath + /roles/headless; # for devices that are of the headless type - provides no GUI
  graphical = modulePath + /roles/graphical; # for devices that are of the graphical type - provides a GUI
  workstation = modulePath + /roles/workstation; # for devices that are of workstation type - any device that is for daily use
  server = modulePath + /roles/server; # for devices that are of the server type - provides online services
  laptop = modulePath + /roles/laptop; # for devices that are of the laptop type - provides power optimizations
